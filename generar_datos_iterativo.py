...
# CONFIGURACIÓN DEL ENTRENAMIENTO ITERATIVO
NUM_PARTIDAS = 1000
PROFUNDIDAD = 3
IA_1 = AgenteMinimax(jugadorIA=1, profundidad_maxima=PROFUNDIDAD, usar_red=True)
IA_2 = AgenteMinimax(jugadorIA=2, profundidad_maxima=PROFUNDIDAD, usar_red=True)

# ALMACENAMIENTO
tableros_totales = []
etiquetas_totales = []

print(f"\nIniciando generación iterativa de {NUM_PARTIDAS} partidas (IA Híbrida vs IA Híbrida)")
...
print(f"\n¡Generación iterativa completada en {round(fin_global - inicio_global, 2)} segundos!")
print(f"Total de tableros de alta calidad recolectados: {len(tableros_totales)}")

# Guardar en memoria con nombres nuevos para no sobreescribir los datos clásicos
np.save("datasets/dataset_iterativo_tableros.npy", np.array(tableros_totales))
np.save("datasets/dataset_iterativo_etiquetas.npy", np.array(etiquetas_totales))